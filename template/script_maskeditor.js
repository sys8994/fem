...
class MaskEditor {
    ...
    async saveData(typ) {
        ...
        if (typ == 'saveas') {
            const oldName = this.maskName + '_copy';
            const newName = await window.prj.customModal.prompt(`Enter copied mask name:`, oldName);
            if (!newName) return;
        ...
        const thumbnail = createThumbnailFromLogicalRect(this.canvas, { x_start: 0, x_end: this.maskWidth, y_start: 0, y_end: this.maskHeight }, this._gridGroup);
        let newMaskData;
        if (!this.id || (typ == 'saveas')) { // new or saveas mode
            const maskId = 'mask' + Date.now().toString().slice(-10);
            newMaskData = {
                id: maskId,
                name: maskname,
        ...
        const procId = window.prj.processFlow.processes.find(p => p.mask === newMaskData.id)?.id
        this.maskmanager.renderMaskList();
        window.prj.customModal.alert('Mask Saved!').then(() => {
            if (procId) window.prj.processFlow.render({ typ: 'maskchange', procId: procId });
        });
        this._setEditStarStatus(false);
    }
