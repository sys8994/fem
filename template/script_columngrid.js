...
class ColumnGrid {
    ...
    initializeCache() {
        this.colsCache = {};
        // this.colsCache[initId] = [null, 0];
        this.sliderCache = {};
    }

    saveCache(id) {
        const nMaxCache = this.nMaxCache;
    ...
    clearCacheExcept(ids) {
        Object.keys(this.colsCache).forEach(k => {
            if (!ids.includes(k)) delete this.colsCache[k];
        });
    }
    ...
    // clearCacheAfter(id) {
    //     Object.keys(this.colsCache).forEach(k => {
    //         if (+k > id) delete this.colsCache[k];
    //     });
    // }
