class Inspector {
    ...
    // Edit 버튼 토글
    toggle(cardEl, proc) {
        const arrow = cardEl.querySelector('.icon-btn-inspectorpanel');
    ...
    rehydrateAll() {
        for (const id of this.expandedIds) {
            const card = this.flow.listEl.querySelector(`.processflow-component[data-id="${id}"]`);
        ...
        return this._makeDropdownRow({
            ...
            onSelect: async (val) => {
                if (val === "+Add") {
                    ...
                    window.prj.gridRenderer.rebuildLegend(this.flow.materialColor);
    ...
    //processflow-component past expanded p_cgmv5kd
    ...
    _updateAnyCardMeta(procId = null) {
        ...
        if (!procId) {
            const cardList = this.flow.listEl.querySelectorAll(`.processflow-component`);
            ...
            const card = this.flow.listEl.querySelector(`.processflow-component[data-id="${procId}"]`);
            if (!card) return;
    ...
    _makeDropdownRow(input) {
        ...
        if (headerType == 'name') {
            ...
            btnContainer.className = 'input-wrapper';
            ...
            btn.addEventListener('change', (e) => {
                this.flow._commitHistory();
                const proc = this.flow.processes.find(p => p.id === procId);
                const cardEl = e.target.closest('.processflow-component');
    ...
    _setBtnArrow(btn, open) {
        ...
        if (arrow) arrow.innerHTML = open ? '<i class="fa-solid fa-chevron-up"></i>' : '<i class="fa-solid fa-chevron-down"></i>';
    }
